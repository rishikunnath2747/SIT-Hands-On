...
module.exports = class JobsService extends cds.ApplicationService {
  ...
  async limiter(limit, payloads, fn, asTask = false) {
    ...
    for (const payload of payloads) {
      const { ID, tenant, job_ID } = payload
      if (asTask) {
        await t0_(async () => {
          await UPDATE(Tasks, { ID, tenant }).with({ status: RUNNING })
          await this.emit('taskUpdate', { task: { ID, job_ID, tenant, status: RUNNING }})
        })
      }
  ...
  async _nextTask(task, _fn) {
    const { ID, tenant, job_ID, op } = task
    try {
      await _fn
      await t0_(async () => {
        await UPDATE(Tasks, { ID, tenant }).with({ status: FINISHED })
        await this.emit('taskUpdate', { task: { ID, job_ID, tenant, op, status: FINISHED }})
      })
    } catch (e) {
      LOG.error(e)
      ...
      await t0_(async () => {
        ...
        await this.emit('taskUpdate', { task: { ID, job_ID, tenant, op, status: FAILED, error }})
      })
    }
  }
}
