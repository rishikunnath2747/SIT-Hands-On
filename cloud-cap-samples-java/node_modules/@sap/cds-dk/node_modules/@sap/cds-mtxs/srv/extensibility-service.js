...
const _lint = async function (tenant, extCsn, tag) {
  ...
  if (cds.env.requires['cds.xt.ExtensibilityService']?.['check-existing-extensions'] === true) {
    // alternative: always use full set of extensions instead of just the new one
    extCsn = await mps.getExtensions(tenant)
  }

  // TODO all existing extensions have to checked as well, at least for limits
