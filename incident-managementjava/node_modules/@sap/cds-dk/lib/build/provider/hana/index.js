...
class HanaBuildPlugin extends InternalBuildPlugin {
    ...
    async build() {
        ...
        if (cds.cli.options?.[OPTION_WS] || hasOptionValue(this.task.options[OPTION_WS], true)) {
            workspaceFiles = await this._getWorkspaceFiles()
        }

        // the order of 1 and 2 is important
        // 1. compile
        const hdiPlugins = await this._compileToHana(model, workspaceFiles)
     ...
     */
    async _compileToHana(model, workspaceFiles) {
        ...
        if (cds.env.features.journal === false
            ...
            return await this._compileToHdbmigration(model, format, workspaceFiles)
        }
    }
    ...
    async _compileToHdbmigration(model, format, workspaceFiles) {
        const hdiPlugins = new Set()
        ...
        for (const { name, suffix, content, changed } of definitions) {
            ...
            if (suffix === FILE_EXT_HDBMIGRATIONTABLE) {
                // Skip the .hdbmigrationtable file if its origin is a different workspace.
                // The workspace build copies artifacts located in 'db/src/**' workspace folders.
                if (!workspaceFiles?.find(wsFile => wsFile.fileName === fileName)) {
    ...
    async _getWorkspaceFiles() {
        ...
        const workspaceFiles = [...dbPaths].reduce((acc, src) => {
            ...
            files.forEach(file => acc.add({ dir: src, filePath: path.relative(src, file), fileName: path.basename(file) }))
            return acc
        }, new Set())
